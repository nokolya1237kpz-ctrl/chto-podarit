                ...
                {product && (
                  <a
                    ...
                    className={`mt-4 inline-flex items-center justify-center rounded-full px-4 py-2.5 text-sm font-semibold transition ${
                      product.affiliateUrl || product.originalUrl
                        ? 'bg-gradient-to-r from-[#7c3aed] to-[#ec4899] text-white hover:brightness-110 shadow-[0_12px_40px_rgba(124,58,237,0.28)]'
                        : 'bg-slate-700/50 text-slate-400 cursor-not-allowed'
                    }`}
                    onClick={(e) => {
                      if (!product.affiliateUrl && !product.originalUrl) {
                        e.preventDefault();
                      }
                    }}
                  >
                    {product.affiliateUrl || product.originalUrl ? 'Посмотреть товар' : 'Ссылка скоро'}
                  </a>
                )}
              </div>
            </div>
