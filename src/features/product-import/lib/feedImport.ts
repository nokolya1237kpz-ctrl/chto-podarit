...
export function parseCsvFeed(text: string): Record<string, string>[] {
  ...
  const delimiters = [';', ',', '\t', '|'];
  const delimiter = firstLine.split(';').length > 4 ? ';' : delimiters.sort((a, b) => firstLine.split(b).length - firstLine.split(a).length)[0];
  const split = (line: string) => line.match(new RegExp(`("([^"]|"")*"|[^${delimiter}]+|(?<=${delimiter})(?=${delimiter}))`, 'g'))?.map((item) => item.trim().replace(/^"|"$/g, '').replace(/""/g, '"')) || [];
