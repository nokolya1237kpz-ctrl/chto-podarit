...
export default function ImportFilePage() {
  ...
  async function preview(nextFile = file) {
    ...
    try {
      ...
      if (!response.ok || !data.success) throw new Error(data.error || 'Preview не выполнен');
  ...
  async function importFile() {
    ...
    try {
      ...
      setMessage(`Импортировано ${data.imported || 0}, draft ${data.drafted || 0}, ошибок ${data.failed || 0}`);
    } catch (err) {
    ...
    <AdminShell title="Import file">
      <div className="space-y-6">
        ...
        <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6">
          <h2 className="text-xl font-semibold">CSV / JSON import</h2>
          <p className="mt-2 text-sm text-slate-400">Для XLSX экспортируйте файл как CSV или JSON. После preview можно сопоставить колонки.</p>
        ...
        {columns.length ? (
          <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6">
            <h2 className="text-xl font-semibold">Mapping columns</h2>
        ...
        {sample.length ? (
          <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6">
            <h2 className="text-xl font-semibold">Preview первых 20 строк</h2>
